...
class DrugController extends CI_Controller {
    ...
    {
        $list = $this->drugs->get_datatables($this->input->get("id"));
        $data = array();
        $no = $_POST['start'];
 ...
 public function fetchc()
    {
        ...
        foreach ($list as $drugs) {
            ...
            $drugimg = $drugs->img;
            if($drugimg == null) $drugimg = "assets/img/defaultdrug.png";  
    ...
    public function addNew(){          
             ...
             $inserted_id = $this->drugs->addNew($array); 


            $arrayPrices = array(
                    'drugprice' => str_replace(",","",$this->input->post("drugprice")),
                    'tax'  => $this->input->post("tax"),
                    'drugid' => $inserted_id,
                    'storeid' => $this->input->get("id")
                ); 

            $this->drugs->addNew_Pricetable($arrayPrices); 

    }

    public function fetchedit(){
        $list = $this->drugs->fetchedit($this->input->post("id") );
    ...
    public function edit(){
               ...
               $this->drugs->saveRecord($array,$this->input->post("id"));
               ...
               $this->drugs->save_Pricetable($arrayPrices,$this->input->post("id")); 

    } 

 public function deleteRecord(){
        $qcd = $this->drugs->deleteRecord($this->input->post("id"));
 }

 public function upload(){

        $upload_handler = new UploadHandler();
       // $additionalFileTask = new SavetoDatabase($uploaded_file, "assets/uploads/excel/PRICELIST OCTOBER 2014.xlsx",$this);
  ...
  public function imgupload(){
        ...
        $upload_handler = new ImgUploadHandler();
 ...
 public function order(){
   ...
   $response = $this->drugs->createTransaction($user,$selString); 
...
public function fetchtransaction(){
       ...
       $selString = $this->input->get("id");
        ...
        $list = $this->drugs->get_pTransactions($user,$selString);
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $drugs) {
            $no++;
            ...
            $row[] = $no;
            $row[] = $drugs->genericname; 
            $row[] = $drugs->transactiondate; 
            $row[] = $drugs->description;      
            $row[] = $drugs->comments;        
            $data[] = $row;
        }
...
public function displayImages(){
    ...
    $drugId = $this->input->post("selId");

    $list = $this->drugs->getImageList($drugId);
    $i = 1;
      foreach ($list as $drugimg) {
       
        $response .= '
                    <div class="holder">
                    <div id="image-'.$i.'" class="image-lightbox">
                    <span class="close"><a href="#">X</a></span>
                    <img src="../'.$drugimg->img.'" alt="preview drug image">
                    <a class="expand" href="#image-'.$i.'"></a>
                    </div>
                    </div>
    ...
    public function filterDrugs(){

        $list = $this->drugs->get_datatablesFilter($this->input->get("id"),$this->input->get("cat"));
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $drugs) {
            $no++;
...
class SavetoDatabase  extends CI_Controller{
       ...
       function __construct($uploaded_file,$file_path,$selId) { 
        ...
        for ($row = 3; $row <= $highestRow; ++$row) {
              ...
              for ($col = 0; $col <= $highestColumnIndex; ++$col) {
                ...
                switch($objWorksheet->getCellByColumnAndRow($col, $row-1)->getValue()){
                    case 'ITEMNAME':$itemname = $col; break;
                    case 'TAX': $tax = $col; break;
                    case 'TRADE PRICE': $price = $col;break; 
                } 

               }           

               $array = array(
                    'genericname' => $rows[$itemname]
                );    

             $inserted_id = $ci->drugs->addNew($array); 


            $arrayPrices = array(
                    'drugprice' => str_replace(",","",$rows[$price]),
                    'tax'  => $rows[$tax],
                    'drugid' => $inserted_id,
                    'storeid' => $this->selId
                ); 

            $ci->drugs->addNew_Pricetable($arrayPrices); 
...
class SaveImgtoDatabase  extends CI_Controller{
       ...
       public function databaseSave(){ 
        ...
        $ci->drugs->saveImage($array,$this->selId);
            ...
            $arrayImage = array(
                    'parent' => $this->selId,
                    'img'  => $this->file_path,
                    'description' => "drug images"
                ); 

            $ci->drugs->addImages($arrayImage); 
...
class UploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {       
        ...
        $this->options = array(
            ...
            'script_url' => 'med-upload',
            //'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/',
            'upload_dir' =>'assets/uploads/excel/',
            'upload_url' => $this->get_full_url().'/assets/uploads/excel/',
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $additionalFileTask = new SavetoDatabase($uploaded_file, $file_path , $_POST['selId']);
                    $additionalFileTask ->databaseSave();
                }
            } else {
...
class ImgUploadHandler
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $selectedrugId = $_POST['selId'];

        $fulpth = $this->get_full_url().'/assets/uploads/drugs/drugs_'.$selectedrugId.'';

        if (!file_exists($fulpth)) {
            mkdir($fulpth, 0777, true);
        }
        ...
        $this->options = array(
            //'script_url' => $this->get_full_url().'/'.$this->basename($this->get_server_var('SCRIPT_NAME')),
            'script_url' => 'med-imgupload',
            //'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/',
            'upload_dir' =>'assets/uploads/drugs/drugs_'.$selectedrugId.'/',
            'upload_url' => $fulpth,
            'input_stream' => 'php://input',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    move_uploaded_file($uploaded_file, $file_path); 
                    $additionalFileTask = new SaveImgtoDatabase($uploaded_file, $file_path , $_POST['selId']);
                    $additionalFileTask ->databaseSave();
                }
            } else {
                // Non-multipart uploads (PUT method support)
