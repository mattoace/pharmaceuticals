 ...
 public function allProducts($from=0,$to=100){  
    ...
    public function autoFill($Parameter){
       ...
       $query = $this->db->query('SELECT d.id,d.genericname,d.serialno,d.productid,dp.drugprice,dp.tax,IF(d.img IS NOT NULL ,concat("https://tibamoja.co.ke/",d.img),"https://tibamoja.co.ke/assets/img/defaultdrug.png") as img
        ...
        LEFT JOIN drugtocategory dtc ON dtc.drugid = d.id
        LEFT JOIN  category c ON c.id = dtc.categoryid   
        WHERE dp.drugid = d.id 
        AND LOWER(d.genericname) LIKE "%'.$Parameter .'%"       
       '); 
  ...
  public function fetchFiles($drugId){   

        $this->db->select(' * ');

        $this->db->from('images');  

        $this->db->where('parent',$drugId);   

        $column_order = array(null, 'images.id','images.img'); 
        $column_search = array('images.id','images.img'); 
        $order = array('images.id' => 'asc');        
...
function count_filtered_img()
   ...
   public function deleteFile($id){
     ...
     $response = $this->db->delete('images'); 
