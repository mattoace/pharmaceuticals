			    ...
			    function formatFileSize(bytes) {
			       ...
			       //datatables
				    childtable = $('#prescriptiontable').DataTable({ 
				        ...
				        // Load data for the table's content from an Ajax source
				        "ajax": {
				            "url": "patientbill-fetch?id=0",  //"url": "<?php echo site_url('patientController/ajax_list')?>",
				            "type": "POST"
				        },
				        		select: {
							style:    'os',
		        ...
		        function addInsu(){
                   ...
                   $.post("patientbill-add",postVars,function(data){

                        alert("Successfully Saved!");
                        loadUrl = "patientbill-fetch?id="+selectedRow;
				        childtable.ajax.url(loadUrl).load();
				        moreSelectEventsChild(data.id);
				        getTotals(selectedRow);
		         ...
		         function deleteInsu(){
		        	if(selectedRowChild){
		        	  ...
		        	  $.post("patientbill-delete",postVars,function(data){
				...
				function moreSelectEvents(){ 
                       ...
                       $("#personimageholderg").attr("height","200px"); 
       	               ...
       	               loadUrl = "patientbill-fetch?id="+selRow[0];	
				...
				function moreSelectEventsChild(selectedPresc){
				 postvars = {"id":selectedPresc}
                  $.post("patientbill-formfetch",postvars,function(data){
                  	$("#oid").val(data.id);
					$("#orderid").val(data.orderid);
					$("#billingdate").val(data.billingdate);
...
function getTotals(id){
	 ...
	 postvars = {"id":id}
                  $.post("patientbill-getotals",postvars,function(data){
