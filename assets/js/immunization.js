		        ...
		        function addInsu(){
		            ...
		            var postVars = {
						...
						"immundate": $("#immundate").val(),
						"clinicattended": $("#clinicattended").val(),						
						"patientid": selectedRow
					}

                   $.post("immunization-add",postVars,function(data){

                        alert("Successfully Saved!");
                        loadUrl = "immunization-fetch?id="+selectedRow;
				...
				function moreSelectEvents(){ 
                       ...
                       $("#personimageholderg").attr("height","200px"); 
       	               ...
       	               loadUrl = "immunization-fetch?id="+selRow[0];	
				...
				function moreSelectEventsChild(selectedPresc){
				 postvars = {"id":selectedPresc}
                  $.post("immunization-formfetch",postvars,function(data){
                  	$("#immun_id").val(data.id);
					$("#immunization").val(data.immunization);
					$("#immundate").val(data.immundate);
					$("#clinicattended").val(data.clinicattended);	       	          
