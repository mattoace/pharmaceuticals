			    ...
			    function formatFileSize(bytes) {
			       ...
			       //datatables
				    prescriptiontable = $('#prescriptiontable').DataTable({ 
				        ...
				        // Load data for the table's content from an Ajax source
				        "ajax": {
				            "url": "patientrefill-fetch?id=0",  //"url": "<?php echo site_url('patientController/ajax_list')?>",
				            "type": "POST"
				        },
				        		select: {
							style:    'os',
				    ...
				    prescriptiontable.on( 'click', 'tr', function (id) { 
						 prescriptiontable.$('tr.selected').removeClass('selected'); 
						 $(this).addClass('selected');                   
				         var selRow = prescriptiontable.row('.selected').data();				     
				         selectedRow = selRow[0];
				         moreSelectEventsPrescription(selectedRow);	 
			         });				
		        ...
		        function deleteRecord(){
		        	  ...
		        	  postVars = {"id": selectedRow}
		        	  $.post("person-delete",postVars,function(data){
				...
				function moreSelectEvents(){ 
                       ...
                       $("#personimageholderg").attr("height","200px"); 
       	               ...
       	               loadUrl = "patientrefill-fetch?id="+selRow[0];	

				       prescriptiontable.ajax.url(loadUrl).load(); 
				}
				...
				function moreSelectEventsPrescription(selectedPresc){
				 postvars = {"id":selectedPresc}
                  $.post("patientrefill-formfetch",postvars,function(data){
                  	$("#medicinename").val(data[0].genericname);
					$("#dosename").val(data[0].dosename);
					$("#dose").val(data[0].dose);
					$("#minimumdose").val(data[0].minimumdose);
					$("#maximumdose").val(data[0].maximumdose);						
                  },"json");
				}
