			    ...
			    function formatFileSize(bytes) {
			       ...
			       //datatables
				    childtable = $('#prescriptiontable').DataTable({ 
				        ...
				        // Load data for the table's content from an Ajax source
				        "ajax": {
				            "url": "patientpayment-fetch?id=0",  //"url": "<?php echo site_url('patientController/ajax_list')?>",
				            "type": "POST"
				        },
				        		select: {
							style:    'os',
		        ...
		        function addInsu(){
                   ...
                   $.post("patientpayment-add",postVars,function(data){

                        alert("Successfully Saved!");
                        loadUrl = "patientpayment-fetch?id="+selectedRow;
		         ...
		         function deleteInsu(){
		        	if(selectedRowChild){
		        	  ...
		        	  $.post("patientpayment-delete",postVars,function(data){
				...
				function moreSelectEvents(){ 
                       ...
                       $("#personimageholderg").attr("height","200px"); 
       	               ...
       	               loadUrl = "patientpayment-fetch?id="+selRow[0];	
				...
				function moreSelectEventsChild(selectedPresc){
				 postvars = {"id":selectedPresc}
                  $.post("patientpayment-formfetch",postvars,function(data){
                  	$("#bid").val(data.id);
...
function getTotals(id){
	 ...
	 postvars = {"id":id}
                  $.post("patientpayment-getotals",postvars,function(data){
