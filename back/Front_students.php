...
$page_settings = array(
  "selectCountSQL" => "SELECT count(id) as totalrows FROM ems_person",
  "selectSQL" => "SELECT p.id,p.fname,p.mname,p.lname,p.address,p.telephone,p.mobile,p.email,p.dor,p.nationalid,p.town,p.gender,p.is_discontinue
                  FROM ems_person p WHERE p.institution = '".$instId."'",
